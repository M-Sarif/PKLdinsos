...
$stmt = $mysqli->prepare("INSERT INTO pengajuan (id_pemohon, nomor_kk, tgl_pengajuan, alamat, rt, rw, desa, kecamatan, sktm, dtks, kartu_keluarga, ktp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

if (!$stmt) {
    die("Error in prepare statement: " . $mysqli->error);
}

$stmt->bind_param("isssssssssss", $id_pemohon, $nomor_kk, $tgl_pengajuan, $alamat, $rt, $rw, $desa, $kecamatan, $sktm, $dtks, $kartu_keluarga, $ktp);
if ($stmt->execute()) {
